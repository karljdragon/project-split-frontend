...
import DelayDocs from './DelayDocs';
...
export default function Delay(props) {
  ...
  async function getPendingList(url) {
    ...
    formData.append('submission_type', 'PE')
  ...
  function displayDocs(email, infoID, columns, rows) {
    ...
    if (email) {
      return (
        <DelayDocs email={email} infoID={infoID} token={props.token}/>
      )
    }else {
      return (
  ...
  return (
      <>
        ...
        <Typography component="h1" variant="h5" align='left'>
          PE Request Document Verification
        </Typography>
        <br/>
